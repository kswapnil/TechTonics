		...
		<div class="row">
			<div class="box">
				...
				<input type="submit" class="btn-danger btn-lg"
					...
					<h3 style="color: blue;" class="text-left">Received Requests</h3>

					<table class="table text-color text-left">
						<thead>
							<tr>
								<th><p style="color: green;">Emp Id</p></th>
								<th><p style="color: green;">Employee Name</p></th>
						...
						<tbody>
						<% ArrayList<TalkRequest> list =Select.getRequestedTalks();						
						for(TalkRequest obj : list)
						{	
						%>
						
								<tr class="default ">
									<td><%=obj.getEmpid()%></td>
									<td><%=obj.getEmpName()%></td>
									<td><%=obj.getTopic()%></td>									
									<td><input type=button class="btn-link " value="Approve" onclick="location.href = 'ApproveTalkController?empid=<%=obj.getEmpid()%>&empname=<%=obj.getEmpName()%>&emptopic=<%=obj.getTopic()%>';">/<input
									type=button class="btn-link " value="DisApprove"></td>
								</tr>
							<%} %>
